...
describe 'Test Admins Handling' do
  ...
  before do
    ...
    @another_account_data = DATA[:accounts][2]
    @wrong_account_data = DATA[:accounts][5]
  ...
  describe 'Adding admins to a department' do
    it 'HAPPY: should add a valid admin to a department' do
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      put "api/v1/organizations/#{@org.id}/departments/#{@dep.id}/admins", req_data.to_json
    ...
    it 'SAD AUTHORIZATION: should not add an admin without authorization' do
      req_data = { email: @another_account.email }
    ...
    it 'BAD AUTHORIZATION: should not add an invalid admin' do
  ...
  describe 'Removing admins from a department' do
    it 'HAPPY: should remove with proper authorization' do
      TrackIt::Api.DB[:accounts_departments]
                  .insert(department_id: @dep.id, employee_id: @another_account.id, role_id: 1)
      req_data = { email: @another_account.email }

      header 'AUTHORIZATION', auth_header(@account_data)
      delete "api/v1/organizations/#{@org.id}/departments/#{@dep.id}/admins", req_data.to_json
    ...
    it 'BAD AUTHORIZATION: should not remove invalid admin' do
      req_data = { email: @another_account.email }
